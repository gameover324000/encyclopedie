...
contenu = Path(FICHIER_Z).read_text(encoding="utf-8")
...
for a in soup.find_all("a", class_="plant-link"):
    ...
    nouveau_href = f"../Plante_page/Z_Plante_page/{slug}.html"
    if a.get("href") != nouveau_href:
        a["href"] = nouveau_href
        corriges += 1

Path(FICHIER_Z).write_text(str(soup), encoding="utf-8")
print(f"✅ {corriges} liens corrigés dans {FICHIER_Z}")
