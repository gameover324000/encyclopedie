...
contenu = Path(FICHIER_W).read_text(encoding="utf-8")
...
for a in soup.find_all("a", class_="plant-link"):
    ...
    nouveau_href = f"../Plante_page/W_Plante_page/{slug}.html"
    if a.get("href") != nouveau_href:
        a["href"] = nouveau_href
        corriges += 1

Path(FICHIER_W).write_text(str(soup), encoding="utf-8")
print(f"✅ {corriges} liens corrigés dans {FICHIER_W}")
