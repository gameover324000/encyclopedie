...
# -*- coding: utf-8 -*-
"""
Herbarium — Liste des produits uniques par catégorie dans un dossier lettre
Même fonctionnement qu'analyse_lettre.py mais sans afficher les plantes.
Génère un fichier .txt listant uniquement les noms de produits uniques.

Usage :
    python liste_produits.py <chemin_dossier_lettre>
    python liste_produits.py ./A_Plante_page

Génère un fichier : produits_<LETTRE>_<DATE>.txt  (dans le dossier analysé)
"""
...
def generer_rapport(resultats, lettre):
    ...
    Format du fichier .txt généré :
        ...
        Thés & Tisanes (3 unique(s)) :
        Tisane pour l'insomnie
        Décoction du matin
        Tisane digestive après repas

        Remèdes & Soins (2 unique(s)) :
        Cataplasme anti-douleur
        Infusion calmante
        ...

    Les catégories sans aucun usage sont omises.
    Chaque produit n'apparaît qu'une seule fois (titres uniques).
    """
    ...
    for cle, config in CATEGORIES.items():
        ...
        if not titres_uniques:
            continue  # catégorie vide → on saute

        lignes.append("")
        lignes.append(f"{config['label']} ({len(titres_uniques)} unique(s)) :")
...
def main():
    print("═" * 55)
    print("  Herbarium — Liste des produits uniques")
    print("═" * 55)
    ...
    nom_log    = f"produits_{lettre}_{date_str}.txt"
