#!/usr/bin/env python3
"""
Herbarium — Correction des liens dans X.html
...
contenu = Path(FICHIER_X).read_text(encoding="utf-8")
...
for a in soup.find_all("a", class_="plant-link"):
    ...
    nouveau_href = f"../Plante_page/X_Plante_page/{slug}.html"
    if a.get("href") != nouveau_href:
        a["href"] = nouveau_href
        corriges += 1

Path(FICHIER_X).write_text(str(soup), encoding="utf-8")
print(f"✅ {corriges} liens corrigés dans {FICHIER_X}")
